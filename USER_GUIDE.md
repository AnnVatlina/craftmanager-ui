...
1. Проверьте количество (по умолчанию 1) и цену (по умолчанию — текущая цена продажи изделия), поправьте при необходимости.
2. Нажмите **«Продать»**.

Продажа создастся автоматически с датой самой ярмарки (а не сегодняшним числом) и привязкой к её каналу — удобно, если вы заносите продажи на следующий день после ярмарки. Каждый клик — отдельная продажа на одно изделие; после сохранения остаток и «Довязать» в таблице обновятся.

Если по изделию на этой ярмарке уже есть хотя бы одна продажа (внесённая как через эту кнопку, так и обычным способом через раздел «Продажи»), вместо полей ввода и кнопки показывается пометка **«✓ Продано»** — это защищает от случайного повторного внесения одной и той же продажи.
