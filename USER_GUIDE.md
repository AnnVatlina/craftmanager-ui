...
| ⚙️ **Настройки** | Подогнать приложение под себя | Валюта, списки категорий и единиц измерения, экспорт/импорт всех данных |
...
### Результаты по ярмаркам

Таблица по каждой ярмарке (каналы с типом «Ярмарка»): дата проведения, сколько изделий было запланировано взять (из подготовки к ярмарке), сколько фактически продано и на какую сумму. Не зависит от фильтра «С / По» выше — всегда показывает итоги за всё время по каждой ярмарке целиком, чтобы можно было сравнить план с фактом.
...
Ниже, в блоке **«История пополнений»**, показаны все партии производства этого изделия (дата, количество, источник — «Производство», «Восстановленная запись» или «Корректировка»), от новых к старым. Ошибочную запись можно поправить (✏️) или удалить (🗑) — остаток изделия на складе пересчитается автоматически, но остатки материалов при этом не трогаются.

### Продажи изделия

Если по изделию уже были продажи, ниже появляется блок **«Продажи»** — по одной строке на каждую продажу, в которой участвовало это изделие (дата, количество, цена, сумма, канал, заметки). Если продаж не было, блок не показывается вовсе.
...
Таблица показывает по одной строке на каждое проданное изделие: дата, изделие, количество, цена и сумма по позиции, заметки, канал продаж. Если в продаже несколько изделий — дата, заметки и канал повторяются в каждой строке, а строки одной продажи визуально сгруппированы. Клик по строке открывает карточку продажи целиком — с итоговой суммой и кнопкой удаления.

Внизу таблицы — строка «Итого» с суммарным количеством и суммой по всем показанным строкам. Если продаж больше одной страницы, строка считает итог только по текущей странице и подписана «Итого на странице».
...
Если продаж больше 20, внизу таблицы появляются кнопки «← Назад» / «Вперёд →». Пагинация работает только при канале «Все» — если выбран конкретный канал, показываются сразу все его продажи одним списком, без разбиения на страницы.
...
| **Продажа** | Быстрая отметка продажи прямо из этой таблицы (см. ниже) |

Сверху — сводка: сколько всего позиций, сколько штук нужно взять всего, и сколько ещё нужно довязать по всем изделиям сразу.

### Быстрая отметка продаж

Когда ярмарка прошла и нужно занести проданное — не обязательно искать каждое изделие в разделе «Продажи». Прямо в этой таблице, в колонке «Продажа»:

1. Нажмите **«Продать»** в строке нужного изделия — откроется окно с уже заполненными названием изделия, датой (дата самой ярмарки, а не сегодняшняя — удобно, если заносите продажи на следующий день) и каналом.
2. Поправьте количество и/или цену, если продали не одну штуку или не по обычной цене — по умолчанию это 1 шт. по текущей цене продажи изделия.
3. Нажмите **«Продать»** в окне.

Каждое такое окно — отдельная продажа на одно изделие; после сохранения остаток и «Довязать» в таблице обновятся.

Если по изделию на этой ярмарке уже есть хотя бы одна продажа (внесённая как через эту кнопку, так и обычным способом через раздел «Продажи»), вместо кнопки «Продать» показывается пометка **«✓ Продано»** — это защищает от случайного повторного внесения одной и той же продажи.
